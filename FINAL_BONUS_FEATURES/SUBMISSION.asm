...
PT_STORE:
    ; We skip overriding temp if we manually received serial for setup
    mov temp_current, r2
    mov temp_tenths, r5
...
Read_Temperature:
    ...
    ; --- STEP 1: READ COLD JUNCTION ---
    lcall Read_LM335
    
    ; --- STEP 2: READ THERMOCOUPLE (ADC0) ---
    Load_x(0)
    mov R5, #5
    
Average_Loop:
    mov ADC_C, #0       ; Force ADC Channel 0
    lcall Wait1ms
    ...
    mov y+1, ADC_H
    mov y+0, ADC_L
    lcall add32         ; Accumulate samples
    djnz R5, Average_Loop
    ...
    ; --- STEP 4: CALCULATE TEMP ---
    Load_y(1000)        ; Convert to microvolts
    lcall mul32
    Load_y(12497)       ; Divide by Gain (320) * Alpha (41)
    lcall div32
    
    ; --- STEP 5: ADD COLD JUNCTION ---
    mov y+0, cj_temp+0
    mov y+1, cj_temp+1
    mov y+2, cj_temp+2
    mov y+3, cj_temp+3
    ...
    ; --- STEP 6: FSM UPDATE & SERIAL ---
    mov temp_current, x+0
    mov temp_tenths, #0
    
    jb  setup_active, RT_SKIP_DISP
    lcall Update7SegTemp
RT_SKIP_DISP:
    setb force_redraw
...
Seconds_Tick:
    ...
    ; --- THIS NOW MEASURES AND SENDS TEMPERATURE EVERY 1 SECOND ---
